...
class TestAlbedo(SMRFConfig, unittest.TestCase):
    ...
    @patch("smrf.distribute.variable_base.ReadNetCDF")
    def test_distribute_file_broadband_nan(self, mock_read_netcdf):

        # Create an array with a NaN value
        values_with_nan = np.array([0.85, np.nan])

        mock_source = MagicMock()
        mock_source.variables = ["albedo"]
        mock_source.load.return_value = values_with_nan
        mock_read_netcdf.return_value = mock_source
        ...
        with self.assertRaisesRegex(ValueError, "NaN"):
            subject.distribute(TIMESTEP, np.array([1.0]), STORM_DAY)

    @patch("smrf.distribute.variable_base.ReadNetCDF")
    def test_distribute_file_direct_diffuse_nan(self, mock_read_netcdf):

        values_direct = np.array([0.80, np.nan])
        values_diffuse = np.array([0.85, 0.85])
